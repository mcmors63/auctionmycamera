        ...
        {tx && (
          <>
            ...
            <div className="mb-4 text-sm text-neutral-700 space-y-1">
              <p><strong>Sale price:</strong> {formatMoney(tx.sale_price)}</p>
              <p><strong>Commission:</strong> {formatMoney(tx.commission_amount)} {typeof tx.commission_rate === "number" ? `(${tx.commission_rate}%)` : ""}</p>
              <p><strong>Seller payout:</strong> {formatMoney(tx.seller_payout)}</p>

              <p><strong>Seller:</strong> {tx.seller_email || "—"}</p>
              <p><strong>Buyer:</strong> {tx.buyer_email || "—"}</p>

              <p><strong>Payment status (Stripe-owned):</strong> {tx.payment_status || "—"}</p>
              <p><strong>Transaction status:</strong> {tx.transaction_status || "—"}</p>

              {tx.stripe_payment_intent_id ? (
                <p className="text-xs text-neutral-600">
                  <strong>PaymentIntent:</strong>{" "}
                  <span className="font-mono">{tx.stripe_payment_intent_id}</span>
                </p>
              ) : null}
              ...
              <p><strong>Created:</strong> {formatDateTime(tx.created_at || tx.$createdAt)}</p>
              <p><strong>Updated:</strong> {formatDateTime(tx.updated_at || tx.$updatedAt)}</p>
