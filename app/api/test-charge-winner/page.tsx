...
export default function TestChargeWinnerPage() {
  const [listingId, setListingId] = useState("AMC-TEST-123");
    ...
    try {
      const res = await fetch("/api/stripe/charge-winner", {
        ...
        body: JSON.stringify({
          ...
          // ✅ Camera policy: no DVLA concept.
          // Keep this field for backward compatibility if the API still expects it.
          dvlaFeeIncluded: true,
        }),
      });
  ...
  return (
    <main className="min-h-screen bg-slate-100 py-10 px-4">
      <div className="max-w-xl mx-auto bg-white rounded-2xl shadow p-6 space-y-6">
        <header>
          <h1 className="text-2xl font-semibold mb-1">Test: Charge Winner (Stripe)</h1>
          <p className="text-sm text-slate-600">
            Admin/dev-only tool to call <code>/api/stripe/charge-winner</code>. Use a Stripe test customer who already
            has a saved card.
          </p>
          <p className="text-xs text-slate-500 mt-2">
            AuctionMyCamera policy: charge the final amount only (no extra fees added here).
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="AMC-1234"
            />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Winner Email (must exist in Stripe)</label>
            <input
              type="email"
              value={winnerEmail}
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Final Amount to Charge (£)</label>
            <input
              type="number"
              min={1}
              step="0.01"
            ...
            <p className="mt-1 text-xs text-slate-500">
              This should match the agreed final price for the winning bid.
            </p>
          </div>
